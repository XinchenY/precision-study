/* ============================================================================
 *  mhd2d_glm_gpu_mixed.cu — 2D OT, STAGE-LEVEL MIXED PRECISION (CUDA)
 *
 *  §10.4 分配方案的实测实现: 状态与存储保持 binary64;
 *  Riemann 通量 (HLLD/HLL) 与守恒更新 (SSPRK2 两级) 在 binary64 计算;
 *  其余阶段 (recovery / CFL 归约 / 面值重构 / rhs 差分 / GLM 阻尼) 将输入
 *  转为 float、以 binary32 运算后写回 double。时间累加器保持 binary64。
 *  与 RAPTOR mode-0 仿真同语义 (逐阶段 cast 而非逐运算 cast)。
 *  基于 mhd2d_glm_gpu.cu, 仅改动 float 阶段的设备函数与三个 kernel。
 *
...
/* ── binary32 阶段的设备函数 (状态仍存 double, 仅计算降精度) ────────────── */
#define F_RHO_FLOOR 1.0e-12f
#define F_P_FLOOR   1.0e-12f
__device__ inline float f32Minmod(float a, float b) {
  if (a * b <= 0.0f) return 0.0f;
  return (fabsf(a) < fabsf(b)) ? a : b;
}
__device__ void f32ConsToPrim9(const float U[MHD_GLM_NVAR],
                               float P[MHD_GLM_NVAR]) {
  float rho = fmaxf(U[MHD_RHO], F_RHO_FLOOR);
  float vx = U[MHD_MX] / rho, vy = U[MHD_MY] / rho, vz = U[MHD_MZ] / rho;
  float bx = U[MHD_BX], by = U[MHD_BY], bz = U[MHD_BZ];
  float v2 = vx*vx + vy*vy + vz*vz;
  float b2 = bx*bx + by*by + bz*bz;
  float p = ((float)MHD_GAMMA - 1.0f)
          * (U[MHD_E] - 0.5f * rho * v2 - 0.5f * b2);
  P[MHD_PR_RHO] = rho; P[MHD_PR_VX] = vx; P[MHD_PR_VY] = vy;
  P[MHD_PR_VZ] = vz;  P[MHD_PR_P] = p;
  P[MHD_PR_BX] = bx;  P[MHD_PR_BY] = by; P[MHD_PR_BZ] = bz;
  P[MHD_GLM_PSI] = U[MHD_GLM_PSI];
}
__device__ void f32PrimToCons9(const float P[MHD_GLM_NVAR],
                               float U[MHD_GLM_NVAR]) {
  float rho = P[MHD_PR_RHO];
  float vx = P[MHD_PR_VX], vy = P[MHD_PR_VY], vz = P[MHD_PR_VZ];
  float bx = P[MHD_PR_BX], by = P[MHD_PR_BY], bz = P[MHD_PR_BZ];
  U[MHD_RHO] = rho;
  U[MHD_MX] = rho * vx; U[MHD_MY] = rho * vy; U[MHD_MZ] = rho * vz;
  U[MHD_E] = P[MHD_PR_P] / ((float)MHD_GAMMA - 1.0f)
           + 0.5f * rho * (vx*vx + vy*vy + vz*vz)
           + 0.5f * (bx*bx + by*by + bz*bz);
  U[MHD_BX] = bx; U[MHD_BY] = by; U[MHD_BZ] = bz;
  U[MHD_GLM_PSI] = P[MHD_GLM_PSI];
}
__device__ inline float f32FastSpeed(const float P[MHD_GLM_NVAR], float bn) {
  float rho = fmaxf(P[MHD_PR_RHO], F_RHO_FLOOR);
  float p = fmaxf(P[MHD_PR_P], F_P_FLOOR);
  float bx = P[MHD_PR_BX], by = P[MHD_PR_BY], bz = P[MHD_PR_BZ];
  float a2 = (float)MHD_GAMMA * p / rho;
  float b2 = (bx*bx + by*by + bz*bz) / rho;
  float can2 = bn * bn / rho;
  float s = a2 + b2;
  float disc = s * s - 4.0f * a2 * can2;
  disc = fmaxf(disc, 0.0f);
  return sqrtf(0.5f * (s + sqrtf(disc)));
}
__device__ inline bool f32PrimUsable(const float P[MHD_GLM_NVAR]) {
  if (P[MHD_PR_RHO] <= F_RHO_FLOOR || P[MHD_PR_P] <= F_P_FLOOR) return false;
  for (int k = 0; k < MHD_GLM_NVAR; k++) {
    if (!isfinite((double)P[k])) return false;
  }
  return true;
}
__device__ void f32ReconPrim9(const float Pm[MHD_GLM_NVAR],
                              const float P0[MHD_GLM_NVAR],
                              const float Pp[MHD_GLM_NVAR],
                              float Pl[MHD_GLM_NVAR],
                              float Pr[MHD_GLM_NVAR]) {
  for (int k = 0; k < MHD_GLM_NVAR; k++) {
    float slope = f32Minmod(P0[k] - Pm[k], Pp[k] - P0[k]);
    Pl[k] = P0[k] - 0.5f * slope;
    Pr[k] = P0[k] + 0.5f * slope;
  }
  if (!f32PrimUsable(Pl) || !f32PrimUsable(Pr)) {
    for (int k = 0; k < MHD_GLM_NVAR; k++) { Pl[k] = P0[k]; Pr[k] = P0[k]; }
  }
}
__device__ void f32LoadPrim(const Real* U9, float P[MHD_GLM_NVAR]) {
  float Uf[MHD_GLM_NVAR];
  for (int k = 0; k < MHD_GLM_NVAR; k++) Uf[k] = (float)U9[k];
  f32ConsToPrim9(Uf, P);
}
...
__device__ void xFaceStatesDev(const Real* U, int i, int j, int nx_total,
  ...
  float Pim[NV], Pi[NV], Pip[NV], Pipp[NV];
  float Pl_i[NV], Pr_i[NV], Pl_ip[NV], Pr_ip[NV];
  f32LoadPrim(&U[(size_t)idx2(i - 1, j, nx_total) * NV], Pim);
  f32LoadPrim(&U[(size_t)idx2(i, j, nx_total) * NV], Pi);
  f32LoadPrim(&U[(size_t)idx2(i + 1, j, nx_total) * NV], Pip);
  f32LoadPrim(&U[(size_t)idx2(i + 2, j, nx_total) * NV], Pipp);
  f32ReconPrim9(Pim, Pi, Pip, Pl_i, Pr_i);
  f32ReconPrim9(Pi, Pip, Pipp, Pl_ip, Pr_ip);
  float Ufl[NV], Ufr[NV];
  f32PrimToCons9(Pr_i, Ufl);
  f32PrimToCons9(Pl_ip, Ufr);
  for (int k = 0; k < NV; k++) { UL[k] = (Real)Ufl[k]; UR[k] = (Real)Ufr[k]; }
}
...
__device__ void yFaceStatesDev(const Real* U, int i, int j, int nx_total,
  ...
  float Pjm[NV], Pj[NV], Pjp[NV], Pjpp[NV];
  float Pl_j[NV], Pr_j[NV], Pl_jp[NV], Pr_jp[NV];
  f32LoadPrim(&U[(size_t)idx2(i, j - 1, nx_total) * NV], Pjm);
  f32LoadPrim(&U[(size_t)idx2(i, j, nx_total) * NV], Pj);
  f32LoadPrim(&U[(size_t)idx2(i, j + 1, nx_total) * NV], Pjp);
  f32LoadPrim(&U[(size_t)idx2(i, j + 2, nx_total) * NV], Pjpp);
  f32ReconPrim9(Pjm, Pj, Pjp, Pl_j, Pr_j);
  f32ReconPrim9(Pj, Pjp, Pjpp, Pl_jp, Pr_jp);
  float Ufl[NV], Ufr[NV];
  f32PrimToCons9(Pr_j, Ufl);
  f32PrimToCons9(Pl_jp, Ufr);
  for (int k = 0; k < NV; k++) { UL[k] = (Real)Ufl[k]; UR[k] = (Real)Ufr[k]; }
}
...
__global__ void speedKernel(const Real* U, int nx, int ny, int ghost,
  ...
  if (c < nx * ny) {
    ...
    int j = ghost + c / nx;
    float P[NV];
    f32LoadPrim(&U[(size_t)idx2(i, j, nx_total) * NV], P);
    float sx = fabsf(P[MHD_PR_VX]) + f32FastSpeed(P, P[MHD_PR_BX]);
    float sy = fabsf(P[MHD_PR_VY]) + f32FastSpeed(P, P[MHD_PR_BY]);
    v = (Real)fmaxf(sx, sy);
  }
  smem[tid] = v;
  __syncthreads();
...
__global__ void rateKernel(const Real* U, int nx, int ny, int ghost,
  ...
  if (c < nx * ny) {
    ...
    float P[NV];
    f32LoadPrim(&U[(size_t)idx2(i, j, nx_total) * NV], P);
    float sx = fmaxf(fabsf(P[MHD_PR_VX]) + f32FastSpeed(P, P[MHD_PR_BX]), (float)ch);
    float sy = fmaxf(fabsf(P[MHD_PR_VY]) + f32FastSpeed(P, P[MHD_PR_BY]), (float)ch);
    v = (Real)(sx / (float)dx + sy / (float)dy);
  }
  smem[tid] = v;
  __syncthreads();
  for (int s = blockDim.x / 2; s > 0; s >>= 1) {
...
__global__ void rhsKernel(const Real* U, const Real* Fx, const Real* Gy,
  ...
  for (int k = 0; k < NV; k++) {
    float d = -((float)Fx[id + k] - (float)Fx[im + k]) / fdx
              - ((float)Gy[id + k] - (float)Gy[jm + k]) / fdy;
    rhs[id + k] = (Real)d;
  }
  rhs[id + MHD_GLM_PSI] = (Real)((float)rhs[id + MHD_GLM_PSI]
      - (float)damping_rate * (float)U[id + MHD_GLM_PSI]);
}
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Reconstruction/time: "
            ...
            << std::endl;
  std::cout << "Precision: mixed (flux+update binary64, other stages binary32,"
            << " binary64 storage)" << std::endl;
  ...
  std::string out_dir = out_env ? std::string(out_env)
                                : std::string("results/mhd/gpu/orszag_tang");
  std::string mk = "mkdir -p " + out_dir;
  if (std::system(mk.c_str()) != 0) { /* ignore */ }

  const char* prec_name = "mixed";
  ...
  out << "# Orszag-Tang ideal MHD with mixed GLM (GPU)\n";
